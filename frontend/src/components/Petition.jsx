...
function Petition({ match }) {
  ...
  // To Do: Test error handling when loading multiple datasets in parralel
  async function fetchSignaturesFromConfig() {
    if (hasGeoConf) {
      ...
      // test code
      // let ret = await Promise.all(pending.flat());
      // ret.push({ label: toLabel("St.Materrn", "SX", 0), data: [] });
      // return ret

...
from application.tests.tracker.conftest import get_geo_key, get_code_key
...
def configure_test_get_signatures_by_locale(cls, **kwargs):
    config = dict(signature_count=range(10_000, 100_000), signatures_by=cls.signatures_by)
...
@pytest.mark.parametrize("class_session",
    [{"func": configure_test_get_signatures_by_locale}],
    indirect=True
)
@pytest.mark.usefixtures("class_session")
@freeze_time(FROZEN_TIME_STR)
class TestGetSignaturesByLocale(TestTrackerViews):
    ...
    locales = {
        "region": {'ons_code': 'M', 'name': 'Scotland'},
        "country": {'code': 'GB', 'name': 'United Kingdom'},
        "constituency":  {'ons_code': 'E14000601', 'name': 'Bristol South'}
    }

    signatures_by = {
            "region":  {"locales":  {"predef": [locales["region"].copy()], "undef": 3}},
            "country": {"locales": {"predef": [locales["country"].copy()], "undef": 3}},
            "constituency":  {"locales": {"predef": [locales["constituency"].copy()], "undef": 3}},
        }


    @classmethod
    def validate_signatures(cls, expectation, result, geography, locale):
        strftime = lambda t: dt.strftime(t, "%d-%m-%YT%H:%M:%S")
        schema_opts = {"many": False, "exclude": ["id", "record", "timestamp", geography]}
        ...
        for expected, actual in zip(expectation, result):
            ...
            expected_signatures = Record.signatures_query([expected], geography, locale).first()
    ...
    def test_get_by_locale(self, geography, timestamp, order, app):
        locale = self.locales[geography]
        code_key = get_code_key(geography)
        code, name = locale[code_key], locale["name"]
        self.run_test(geography, code, timestamp, order, app)
        self.run_test(geography, name, timestamp, order, app)

    def run_test(self, geography, locale, timestamp, order, app):
        route = self.base_route + f"{geography}/{locale}"
        params = {}
        if order:
            params["order"] = order
        ...
        assert self.validate_signatures([expected_latest_record], [latest_data], geography, locale)
        assert self.validate_signatures(expected_records, data["signatures"], geography, locale)

    def test_no_signatures_found_returns_404(self, app, petition_id=999, geography="country", locale="IM"):
        route = f"/petition/{petition_id}/signatures_by/{geography}/{locale}"
        response = app.test_client().get(route)
        assert self.validate_status(response, 404)

    def test_petition_not_found_returns_404(self, app, petition_id=999, geography="country", locale="GB"):
        route = f"/petition/{petition_id}/signatures_by/{geography}/{locale}"
        response = app.test_client().get(route)
        assert self.validate_status(response, 404)

    def test_invalid_geography_returns_400(self, app, geography="invalid", locale="GB"):
        route = self.base_route + f"{geography}/{locale}"
        response = app.test_client().get(route)
        assert self.validate_status(response, 400)

    def test_invalid_locale_returns_400(self, app, geography="country", locale="invalid"):
        route = self.base_route + f"{geography}/{locale}"
        response = app.test_client().get(route)
        assert self.validate_status(response, 400)